...
sourcetonames = pd.DataFrame(
    cursor.fetchall(), columns=['name_id', 'source_id'])
...
measurements = pd.DataFrame(
    cursor.fetchall(),
    columns=['measurement_id', 'catalogue_id', 'source_id', 'RA', 'RA_uncer',
             'DEC', 'DEC_uncer', 'frequency', 'flux', 'flux_uncer', 'degree',
             'degree_uncer', 'angle', 'angle_uncer', 'extension',
             'fluxratio', 'origin', 'date_observed', 'date_created', 'valid',
             'uvmin', 'uvmax'])
...
# create data frame with only alma measurements
...
# create data frame merging vlbi_j2 with alma_measurements,
# a full table with the addition of vlbi information for the
# sources that are in the vlbi  by name

alma_vlbi = pd.merge(alma_measurements, vlbi_j2, on='source_id', how='inner')

# lists with source_id of alma_measurements and alma_in_vlbi
...
# The sign of the DEC is gather from the original DEC column, assuming that
# the difference is always small (This because when importing rfc_vlbi
# catalog '-0' is not detected as a negative value...)

alma_vlbi['RA_vlbi'] = alma_vlbi['ra_h'] * 15. + alma_vlbi['ra_m'] * 15. / 60.\
    + alma_vlbi['ra_s'] * 15. / 3600.
alma_vlbi['DEC_vlbi'] = alma_vlbi['dec_d'] + alma_vlbi['dec_m'] * \
    pd.np.sign(alma_vlbi['dec_d']) / 60. + alma_vlbi['dec_s'] * \
    pd.np.sign(alma_vlbi['dec_d']) / 3600.
d0 = alma_vlbi.query('dec_d == 0')

alma_vlbi.loc[d0.index.tolist(), 'DEC_vlbi'] = \
    (alma_vlbi.loc[d0.index.tolist(), 'dec_d'] +
     alma_vlbi.loc[d0.index.tolist(), 'dec_m'] / 60. +
     alma_vlbi.loc[d0.index.tolist(), 'dec_s'] / 3600.) * \
    pd.np.sign(alma_vlbi.loc[d0.index.tolist(), 'DEC'])

# Creating columns with RA and DEC differences, in arcsec.
alma_vlbi['DEC_diff'] = 3600. * np.abs(alma_vlbi['DEC'] - alma_vlbi['DEC_vlbi'])
alma_vlbi['RA_diff'] = 3600. * \
    np.abs((alma_vlbi['RA'] - alma_vlbi['RA_vlbi']) * np.cos(alma_vlbi['DEC']))
...
alma_vlbi_latest = alma_group.apply(
    lambda t: t[t.date_observed == t.date_observed.max()])
alma_vlbi_latest = alma_vlbi_latest.drop_duplicates(
    cols=['source_id', 'date_observed'])
alma_vlbi_latest.to_excel('/home/itoledo/Downloads/latestmeasurements.xls')

pd.options.display.max_columns = 40

# New dataframe to be updated in the archive
alma_vlbi.ra_err /= 3600000.
alma_vlbi.dec_err /= 3600000
new_alma = alma_vlbi[
    [u'measurement_id', u'catalogue_id', u'source_id', u'RA_vlbi', u'ra_err',
     u'DEC_vlbi', u'dec_err', u'frequency', u'flux', u'flux_uncer',
     u'degree', u'degree_uncer', u'angle', u'angle_uncer', u'extension',
...
# g4 = non_grid.query('frequency < 1.30e+11 and RA >=30 and RA < 60')[[
#      'source_id', 'name', 'ra_h', 'ra_m', 'ra_s', 'dec_d', 'dec_m', 'dec_s',
#      'frequency', 'flux']]
